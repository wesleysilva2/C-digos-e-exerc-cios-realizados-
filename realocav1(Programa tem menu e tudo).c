...
int* realoca(int* ptr, int* size) //realoca o ponteiro ptr r o ponteiro size.
{
 ...
 ptr = (int *) realloc(ptr, (*size + add)*sizeof(int) ); //o valor fornecido pelo usuário é somado ao valor do tamanho passado por argumento multiplicaod pelo tamnaho do inteiro, o resultado disso é alocado na variavel ptr.
  ...
  *size += add; // += e operação de soma ou seja *size + add
 }
 else
 {
